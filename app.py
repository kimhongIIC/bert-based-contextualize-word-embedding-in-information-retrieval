...
def load_resources():
    """
    Loads the model, embeddings, and dataset with caching.
    """
    try:
        model = SentenceTransformer(r'saved_model/sbert_model')
        title_embeddings = torch.load(r'saved_model/title_embeddings_v2.pt')
        data = pd.read_csv('updated500k_arXivDataset.csv')
        return model, title_embeddings, data
...
def search_titles(user_query, top_k=5):
    ...
    try:
        ...
        model, title_embeddings, data = load_resources()
        ...
        print(f"Error in search_titles: {str(e)}")
        return [("An error occurred during the search process.", 0)]
...
def index():
    ...
    try:
        ...
        return render_template("index.html", query=None, results=[("An error occurred.", 0)])
